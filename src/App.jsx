        ...
        {enrolledExamObjs.map(exam=>(
          <div key={exam.id} style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:16, padding:18, marginBottom:12 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"repeat(5,1fr)", gap:6 }}>
              {DAY_OPTIONS.map(opt=>{
                const sel=examPlans[exam.id]===opt.days;
                return (
                  <Btn key={opt.days} onClick={()=>{ setExamPlans(p=>({...p,[exam.id]:opt.days})); showToast("Plan updated!"); }}
                    ...
                    <div style={{ fontSize:16, fontWeight:900, color:sel?opt.color:T.text }}>{opt.days}</div>
